...
class OSMService:
    ...
    async def validate_and_get_coordinates(self, place_name: str, city: str) -> Optional[Dict[str, float]]:
        ...
        try:
            # Try cache with variants
            cached = await LocationCache.find_by_variants(place_name, city)
            if cached:
            ...
            # Try direct OSM lookup
            self.logger.debug(f"Cache miss, attempting OSM lookup for {place_name}")
            ...
            if result:
                ...
                return {
                    ...
                    "translations": [place_name]
                }

            # Try alternate names
            alternates = await self._get_alternate_names(place_name, city)
            if alternates:
                lat, lon, confidence, display_name = alternates
                await self._cache_result(
                    place_name, 
                    city, 
                    (lat, lon), 
                ...
                return {
                    ...
                    "longitude": lon,
                    "original_name": place_name,
                    "translations": [place_name, display_name]
                }
    ...
    async def _geocode_location(self, query: str) -> Optional[Tuple[float, float, str]]:
        ...
        if result and len(result) > 0:
            ...
            return (
                float(result[0]["lat"]), 
                float(result[0]["lon"]),
                result[0].get("display_name", "").split(',')[0]
            )
    ...
    async def _get_alternate_names(self, place_name: str, city: str) -> Optional[Tuple[float, float, float, str]]:
        ...
        try:
            ...
            if best_match:
                ...
                self.logger.info(f"Found best alternate match: {display_name} ({best_similarity})")
                return (
